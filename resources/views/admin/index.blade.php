...
@php
	$date = date('d-m-y');
	$status = 'delivered';
	$today = App\Models\Order::where('order_date',$date)->where('status',$status)->sum('amount');

	$month = date('F');

	$month = App\Models\Order::where('order_month',$month)->where('status',$status)->sum('amount');

    $year = date('Y');
	$year = App\Models\Order::where('order_year',$year)->where('status',$status)->sum('amount');
...
<div class="box overflow-hidden pull-up">
	<div class="box-body">							
		...
		<div>
			<p class="text-mute mt-20 mb-0 font-size-16">Đơn hàng chờ giải quyết </p>
			<h3 class="text-white mb-0 font-weight-500">{{ count($pending) }} <small class="text-danger"><i class="fa fa-caret-down	"></i> Đang chờ </small></h3>
		</div>
	</div>
</div>
</div>
